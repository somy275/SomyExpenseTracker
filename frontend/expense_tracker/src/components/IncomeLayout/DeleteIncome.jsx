...
import { DeleteIncomeData, GetIncome } from '../../Store/incomeSlice'
import { useNavigate } from "react-router"
import { GetTotalIncomeExpense } from '../../Store/TotalIncomeExpense'

const DeleteIncome = ({ setShowDeleteIncomePage, ShowDeleteIncomePage, DeleteIncomeId }) => {
    const dispatch = useDispatch()
    const navigate = useNavigate()
    const onDeleteData = async () => {
        const res = await dispatch(DeleteIncomeData(DeleteIncomeId))
        if (DeleteIncomeData.fulfilled.match(res)) {
            dispatch(GetIncome())
            dispatch(GetTotalIncomeExpense())
            navigate("/income");
            setShowDeleteIncomePage(false)
        }
    }
    return (
        ShowDeleteIncomePage && (
            <div className='fixed inset-0 bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-50'>
                <div className='relative bg-white p-4 rounded-xl shadow-2xl w-full max-w-md overflow-y-auto max-h-[95%]'>
                    <div className='relative flex justify-between items-center text-md font-medium h-fit'>
                        <h3 className='text-md'>Delete Income</h3>
                        <button className='text-gray-500 text-xl cursor-pointer' type='button'>
                            <IoClose onClick={() => setShowDeleteIncomePage(false)} />
                        </button>
                    </div>
                    <div className='absolute w-full h-fit left-0 right-0 bottom-[67%] border-b-[1.25px] border-b-gray-300'></div>
                    <div className='flex flex-col mt-5 gap-6'>
                        <h3 className='text-[.8rem] font-medium'>Are you sure you want to delete this income detail?</h3>
